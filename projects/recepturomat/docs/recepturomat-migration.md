...
The target workspace is the Nx 23 project-template structure in this repository.
...
Adapt the generated project under `projects/recepturomat/`. Do not copy the old
repository layout.

## Architecture Decision

Use project-local SQLite for the migrated Recepturomat runtime.
...
- This repository already uses project-scoped SQLite for platform auth and
...
- Recipe-specific data access, schema bootstrap, and feature routes stay
  project-local until reuse is proven.
- Project config uses Recepturomat-specific project ID, database paths, seed
  flags, SSR paths, and frontend routing settings.

For the first SQL model, store recipes in a project-local table with stable
...
- [#68](https://github.com/ksojecki/rod-manager/issues/68) - Define
  project-local recipe SQL schema and store.
...
- [#72](https://github.com/ksojecki/rod-manager/issues/72) - Document project
...
- Do not make Recepturomat share Rod Manager's project database, users, or
  runtime identity.
